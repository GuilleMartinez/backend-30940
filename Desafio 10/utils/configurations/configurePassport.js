module.exports = () => {
    ...
    const loginStrategy = new Strategy({ usernameField: "email" }, async (username, password, done) => {
        ...
        const validPassword = user ? await isValidPassword(user, password) : false;
    ...
    const signupStrategy = new Strategy({ passReqToCallback: true, usernameField: "email" }, async (req, username, password, done) => {
        ...
        if (searchingError) return done(searchingError);
            ...
            const { added, error: creationError } = await users.insertOne({ ...req.body, password });
